...
- CRITICAL — Available resume languages (codes for [DOWNLOAD_RESUME]): ${availableResumeLangs.length ? availableResumeLangs.join(', ') : 'none'}. When telling the user, use these names: en = English/영문, ko = Korean/한국어, it = Italian/이탈리아어. You MUST only offer or suggest download for these languages. If the user asks for a language NOT in this list, say clearly that the resume is only available in [list the available ones by name, e.g. "영문과 한국어"], and do NOT output [DOWNLOAD_RESUME] for unavailable languages.
...
- When the user asks "이력서 어디 있어?", "where is the resume?", "이력서 알려줘", "한국어도 있어?": answer that the resume is PDF download only (no page). Tell them which languages are available by name (e.g. "영문과 한국어로 준비되어 있습니다" when en and ko are available). Ask "다운받을까요?" only for an available language. When they confirm, output [DOWNLOAD_RESUME: XX] only for XX in the available list.
- When in a LATER message the user CONFIRMS they want to DOWNLOAD the resume: output [DOWNLOAD_RESUME: XX] only if XX is one of: ${availableResumeLangs.length ? availableResumeLangs.join(', ') : 'none'}. Otherwise reply that the resume is not available in that language and list the available ones by name. All judgment by you from context and user language.
